...
__host__ int main() {
  ...
  const char* tests[] = {/*0*/ "build/sqrt_pow.so",
  ...
  // When set true, the profiler(eval_cosched_time) will run a subset of all the
  // subkernels to estimate the execution time.
  bool sampling = true;
  ...
  CoSchedKernels co_kernel{kernel_1, kernel_2, streams[0], streams[1]};
  ...
  printf("============================== Not sliced Duration %lf\n",
         co_kernel.eval_cosched_time(co_kernel.get_boundary(), nrepeat, false,
                                     false, false, nullptr, 128));
  ...
  printf("Config %d, %d\n", boundary.first / granularity.first,
         boundary.second / granularity.second);
...
#ifdef SHOW_TRACE_EXP
  for (auto current : {Config{granularity * Config{9, 44}}})
#else
  for (auto current :
       {
...
#endif
  {
...
#ifdef SHOW_TRACE_EXP
    Config step{20, 20};
    auto next_step = [&](Config prev_step) {
      return Config{// step.first > min_step? step.first / 2 : 1,
    ...
    auto next_step = [&](Config prev_step) {
      return Config{
          // step.first > min_step
          //                 ? step.first / std::max(boundary.first / 512 / 4,
          //                 2)
          //                 : min_step,
...
#endif
    for (Config radius{}; step.first >= min_step || step.second >= min_step;
         // radius = step * 2,
         step = next_step(step)) {
      auto res =
      ...
      printf("Sampled opt config %d %d (step %d %d)\n",
             current.first / granularity.first,
             current.second / granularity.second, step.first, step.second);
    }
    auto sampling_end_time = current_seconds();
    sampling_time += sampling_end_time - sampling_start_time;
    double true_time = co_kernel.eval_cosched_time(current, nrepeat, false,
                                                   false, false, nullptr, 32);
    ...
    printf(
        ...
        "%u\n",
        current.first, current.second, current_time, true_time, stat.steps,
        stat.cache_hit);
  }

  printf("Time taken to do searching: %lf\n", sampling_time);
  auto& res =
      *std::min_element(opt_results.begin(), opt_results.end(),
