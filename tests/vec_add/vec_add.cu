...
__global__ void
vec_add(Args args, KernelSlice kernel_slice)
{
    dim3 blockIdx = kernel_slice.get_original_block_idx();
...
EXPORT KernelConfig pre_process()
{
    ...
    CHECK(cudaMemcpy(vecAdd_d_A, vecAdd_h_A, vecAdd_size, cudaMemcpyHostToDevice));
    CHECK(cudaMemcpy(vecAdd_d_B, vecAdd_h_B, vecAdd_size, cudaMemcpyHostToDevice));
